...
var sensitiveValuePatterns = []*regexp.Regexp{
	regexp.MustCompile(`(?i)\b((?:password|passwd|pwd|token|access_token|refresh_token|api[_-]?key|secret|session)=)([^\s&;"']+)`),
	regexp.MustCompile(`(?i)\b(authorization:\s*(?:bearer|basic)\s+)([^\s"']+)`),
	regexp.MustCompile(`(?i)\b(cookie:\s*[^\r\n]*?(?:session|token|auth)[^=]*=)([^\s;"']+)`),
}
