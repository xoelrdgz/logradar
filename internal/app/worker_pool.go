...
func (wp *WorkerPool) worker(ctx context.Context, id int) {
	...
	for {
		...
		case entry, ok := <-wp.inputChan:
			...
			if wp.allowlist != nil && wp.allowlist.Allows(entry) {
				for _, obs := range wp.observers {
					obs.IncrementLinesProcessedByResult("allowlisted")
				}
				if wp.metrics != nil {
			...
			for _, detector := range wp.detectors {
				...
				if result.Detected {
					result = wp.scorer.Score(entry, result)
					if result.Audit {
						for _, obs := range wp.observers {
							obs.IncrementLinesProcessedByResult("audit")
						}
						lineHasThreat = true
						continue
					}
					...
					alert.ApplyDetectionResult(result)
		...
		case <-timer.C:
			if wp.overflow != nil && wp.overflow.Enabled() {
				if err := wp.overflow.WriteAlert(alert); err != nil {
					...
					wp.notifyProcessingError("overflow_write_failed")
					return false
				}
				wp.overflowAlerts.Add(1)
				wp.notifyQueueOverflow()
				return true
			}
			wp.notifyProcessingError("overflow")
			return false
		}
	}
		...
		case alert, ok := <-wp.outputChan:
			...
			for _, alerter := range wp.alerters {
				if err := alerter.Send(ctx, alert); err != nil {
					...
					wp.notifyAlerterError(fmt.Sprintf("%T", alerter))
				}
			}
...
func (wp *WorkerPool) Submit(entry *domain.LogEntry) bool {
	...
	if wp.useBackpressure {
		...
		case <-timer.C:
			...
			wp.notifyProcessingError("timeout")
			return false
		}
	}
...
func (wp *WorkerPool) notifyProcessingError(errorType string) {
	wp.mu.RLock()
	observers := append([]ports.ProcessingObserver(nil), wp.observers...)
	wp.mu.RUnlock()
	for _, obs := range observers {
		if detailed, ok := obs.(ports.DetailedProcessingObserver); ok {
			detailed.IncrementProcessingErrors(errorType)
		}
	}
}

func (wp *WorkerPool) notifyQueueOverflow() {
	wp.mu.RLock()
	observers := append([]ports.ProcessingObserver(nil), wp.observers...)
	wp.mu.RUnlock()
	for _, obs := range observers {
		if detailed, ok := obs.(ports.DetailedProcessingObserver); ok {
			detailed.IncrementQueueOverflow()
		}
	}
}

func (wp *WorkerPool) notifyAlerterError(output string) {
	wp.mu.RLock()
	observers := append([]ports.ProcessingObserver(nil), wp.observers...)
	wp.mu.RUnlock()
	for _, obs := range observers {
		if detailed, ok := obs.(ports.DetailedProcessingObserver); ok {
			detailed.IncrementAlerterErrors(output)
		}
	}
}
