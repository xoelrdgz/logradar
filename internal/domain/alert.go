...
func (a *Alert) ApplyDetectionResult(result DetectionResult) {
	a.RuleID = result.RuleID
	a.RuleVersion = result.RuleVersion
	a.Confidence = clampConfidence(result.Confidence)
	a.Evidence = sanitizeEvidence(result.Evidence)
}

func truncateRawLog(rawLog string) string {
	if len(rawLog) <= MaxLineLength {
		return rawLog
	}
	return rawLog[:MaxLineLength]
}
...
func sanitizeEvidence(evidence Evidence) Evidence {
	const maxEvidenceFragmentLength = 256
	if len(evidence.Fragment) > maxEvidenceFragmentLength {
		evidence.Fragment = evidence.Fragment[:maxEvidenceFragmentLength]
	}
	return evidence
}
